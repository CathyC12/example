...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
	...
	*/
    ...
    oponent = true;
      ...
      JOptionPane.showMessageDialog(null,"White Has Taken Your King! Game Over");
      System.exit(0);
		}
  }
		else{
  ...
  private Boolean checkBlackOponent(int newX, int newY){
		...
		if(((tmp1.contains("White")))){
			oponent = true;
      if(tmp1.contains("King")){
      JOptionPane.showMessageDialog(null,"Black Has Taken Your King! Game Over");
      System.exit(0);
    }
		}
		else{
  ...
  private String getPieceName(int x, int y){ // similar to the piece present method but instead of returning a boolean in is returing the name of the piece on the tile we want to check
    Component c = chessBoard.findComponentAt(x, y);
  ...
  private Boolean KingNear(int x, int y){
    if(((piecePresent(x, y + 75)) && getPieceName(x, y + 75).contains("King")) || //Checking the tile ahead while moving down the board
    ((piecePresent(x, y - 75)) && getPieceName(x, y - 75).contains("King")) || //Checking the tile ahead while moving up the board
    ((piecePresent(x + 75, y)) && getPieceName(x + 75, y).contains("King")) || //Checking the tile ahead while moving to the right
    ((piecePresent(x - 75, y)) && getPieceName(x - 75, y).contains("King")) || //Checking the tile ahead while moving to the left
    ((piecePresent(x + 75, y + 75)) && getPieceName(x + 75, y + 75).contains("King")) || //Checking the southwest path ahead
    ((piecePresent(x + 75, y - 75)) && getPieceName(x + 75, y - 75).contains("King")) || // Checking the northwest path ahead
    ((piecePresent(x - 75, y + 75)) && getPieceName(x - 75, y + 75).contains("King")) || // Checking the souteast path ahead
    ((piecePresent(x - 75, y - 75)) && getPieceName(x - 75, y - 75).contains("King"))){ // checking the northeast path ahead
	                ...
	                if(((landingX < 0) || (landingX > 7))||((landingX<0) || (landingY > 7))){
	                  ...
	                  if((xMovement<=1) && (yMovement<=1)){//if movement is <=1 in any direction then ok
                        ...
                        whiteTurn = true;
	                          ...
	                          if(checkBlackOponent(e.getX(), e.getY())){//check if its oponet piece
  ...
  //Black Bishup
  ...
  } //closes not white turn
else{ // if its not black piece turn do this
    //White Queen
    ...
    //White Pawn code
		...
		if(progression){
          ...
          else if(success){ // the pwan changes into a queen
